...
{
    /// <summary>
    /// The date stamped on an entry whose real date is not known, which is every
    /// <see cref="PlaybackHistorySource.Imported"/> entry bar the most recent one for a given item.
    /// It has to be a date no real session can occupy, so that these entries fall outside every
    /// window a statistics query can ask for and never read as recent activity. Anything projecting
    /// a "last played" date has to map it back to <c>null</c> rather than report 1970.
    /// </summary>
    public static readonly DateTime UnknownDate = DateTime.UnixEpoch;
    ...
    /// <summary>
    /// Gets or sets where this entry came from. Imported entries were reconstructed from the
    /// pre-existing watched status and carry no timing or delivery detail, so statistics that
    /// describe *when* or *how* something was watched have to exclude them.
    /// </summary>
    public PlaybackHistorySource Source { get; set; }
