...
public class PlaybackHistoryManager : IPlaybackHistoryManager
{
    ...
    private readonly ConcurrentDictionary<string, long> _pendingTransferredBytes = new(StringComparer.Ordinal);
    ...
    public void ReportTransferredBytes(string playSessionId, long bytes)
    {
        ...
        _pendingTransferredBytes.AddOrUpdate(playSessionId, bytes, (_, existing) => existing + bytes);
    }
    ...
    public async Task<PlaybackStatsSummaryDto> GetStatsSummaryAsync(DateTime? startDate, DateTime? endDate, Guid? userId, string? mediaType, CancellationToken cancellationToken = default)
    {
        ...
        await using (dbContext.ConfigureAwait(false))
        {
            ...
            // Data transferred: prefer the measured bytes; fall back to the bitrate-based estimate.
            var measuredBytes = await query
                .Where(h => h.ActualBytesTransferred != null)
                .SumAsync(h => (double?)h.ActualBytesTransferred, cancellationToken).ConfigureAwait(false) ?? 0;
            var estimatedBits = await query
                .Where(h => h.ActualBytesTransferred == null && h.Bitrate != null)
                .SumAsync(h => (double)h.Bitrate!.Value * h.PlayedDurationTicks, cancellationToken).ConfigureAwait(false);
            ...
            return new PlaybackStatsSummaryDto
            {
                ...
                TotalDataTransferredBytes = (long)(measuredBytes + (estimatedBits / TimeSpan.TicksPerSecond / 8))
            };
        }
    }
