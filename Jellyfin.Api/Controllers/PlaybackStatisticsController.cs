...
public class PlaybackStatisticsController : BaseJellyfinApiController
{
    ...
    /// <param name="utcOffsetMinutes">Optional. Viewer offset from UTC, deciding which calendar day a session counts towards.</param>
    ...
    public async Task<ActionResult<PlaybackStatsSummaryDto>> GetPlaybackStatsSummary(
        ...
        => await _playbackHistoryManager.GetStatsSummaryAsync(startDate, endDate, userId, mediaType, utcOffsetMinutes, cancellationToken).ConfigureAwait(false);
    ...
    public async Task<ActionResult<IReadOnlyList<PlaybackStatsTimelineEntryDto>>> GetPlaybackStatsTimeline(
        ...
        CancellationToken cancellationToken = default)
    {
        var result = await _playbackHistoryManager.GetStatsTimelineAsync(startDate, endDate, userId, mediaType, interval, utcOffsetMinutes, cancellationToken).ConfigureAwait(false);
        return Ok(result);
    }
    ...
    /// <param name="userId">Optional. Scope to a single user.</param>
    /// <param name="mediaType">Optional. Scope to a media type (Movie, Episode, Audio, …).</param>
    /// <param name="utcOffsetMinutes">Optional. Viewer offset from UTC. Leave at 0 to read the cells as UTC.</param>
    /// <param name="cancellationToken">The cancellation token.</param>
    ...
    public async Task<ActionResult<IReadOnlyList<PlaybackStatsHeatmapEntryDto>>> GetPlaybackStatsHeatmap(
        ...
        CancellationToken cancellationToken = default)
    {
        var result = await _playbackHistoryManager.GetHeatmapAsync(startDate, endDate, userId, mediaType, utcOffsetMinutes, cancellationToken).ConfigureAwait(false);
        return Ok(result);
    }
    ...
    public async Task<ActionResult<IReadOnlyList<PlaybackHistoryDto>>> GetPlaybackStatsSessions(
        ...
        CancellationToken cancellationToken)
    {
        // An activity feed: imported entries have no device, client, or time of day to show.
        var history = await _playbackHistoryManager
            .GetHistoryAsync(userId, null, startDate, endDate, mediaType, limit, includeImported: false, cancellationToken)
