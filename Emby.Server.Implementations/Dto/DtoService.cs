...
namespace Emby.Server.Implementations.Dto
{
    public class DtoService : IDtoService
    {
        ...
        /// <summary>
        /// Reports the by-name items behind the names an item carries, in the order the item carries them.
        /// </summary>
        /// <remarks>
        /// Matching is on the clean value, because two spellings are one genre. A name no link claims is
        /// still reported under the id its own spelling hashes to: a link that never resolved is not the
        /// same as the genre being gone, and dropping it would take the name off the item entirely. A link
        /// that claims no name means the by-name item was renamed, and then the links are the only
        /// truthful answer, so the unclaimed names are left out rather than reported twice.
        /// </remarks>
        /// <param name="names">The names the item carries.</param>
        /// <param name="linked">The by-name items it links to, if any were fetched.</param>
        /// <param name="idFor">Hashes a name to the id a by-name item of that name would take.</param>
        /// <returns>The name and id of every genre or studio the item should report.</returns>
